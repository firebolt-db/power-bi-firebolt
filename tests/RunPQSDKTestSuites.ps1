<#
       ...
       .PARAMETER y
       Optional parameter to bypass all user prompts and assume affirmative responses for automated execution
       ...
       .EXAMPLE 
       PS> .\RunPQSDKTestSuites.ps1 -y

       .EXAMPLE 
       PS> .\RunPQSDKTestSuites.ps1 -y -DetailedResults
#>
...
if (!$y) {
    Write-Host("In DO YOU WANT TO PROCEED? block")
...
foreach ($TestSettings in $TestSettingsList){
    ...
    Write-Host("Running tests for settings file: " + $TestSettings)
    if ($ValidateQueryFolding) { 
        $RawTestResult = & $PQTestExePath compare -p -e $ExtensionPath -sf $TestSettingsDirectoryPath\$TestSettings -dfp $DiagnosticFolderPath
    }
    else {
        $RawTestResult = & $PQTestExePath compare -p -e $ExtensionPath -sf $TestSettingsDirectoryPath\$TestSettings -fomof
    }
    ...
    foreach($Result in $TestResultsObject){
           Write-Host("Processing test result for: " + $Result.Name)
